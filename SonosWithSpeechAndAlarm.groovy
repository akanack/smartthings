 ...
 *  Edited by CAB426 to add Alarm capability
 */

metadata {
    preferences {
       input "alarmMP3", "string", title: "HTTP path for MP3 file",
              description: "This is the HTTP path to the MP3 file that is being used as the alarm sound.", defaultValue: "http://s3.amazonaws.com/smartapp-media/sonos/alarm.mp3",
              required: true, displayDuringSetup: true
	   input "alarmVolume", "number", title: "Alarm Volume (1-100)",
              description: "This is the volume of the alarm.", defaultValue: 100,
              required: true, displayDuringSetup: true
    }


	definition (name: "Sonos Player with Alarm", namespace: "smartthings", author: "SmartThings") {
		capability "Actuator"
		capability "Switch"
		capability "Refresh"
		capability "Sensor"
	...
	standardTile("alarm", "device.alarm", width: 1, height: 1) {
		state "off", label:'off', action:'toggleAlarm', icon:"st.alarm.alarm.alarm", backgroundColor:"#ffffff", nextState:"..."
		state "on", label:'on', action:'toggleAlarm', icon:"st.alarm.alarm.alarm", backgroundColor:"#e86d13", nextState:"..."
		state "both", label:'on', action:'toggleAlarm', icon:"st.alarm.alarm.alarm", backgroundColor:"#e86d13", nextState:"..."
	}
...
def playTrackAndRestore(uri, duration, volume=null) {
	...
	coordinate({
		...
		if (currentTrack) {
			def delayTime = ((duration as Integer) * 1000 * 10)+3000
			if (level) {
				delayTime += 1000
			}
			result << delayAction(delayTime)
...
def alarmOn() {
	log.trace "alarmOn"
    //TODO: Figure out why SONOS won't play twice.
   
    soundAlarm(settings.alarmMP3, settings.alarmVolume)
 	
}

def toggleAlarm() {
	log.trace "Toggling Alarm - Current Value: " + state.alarmStatus
	
	if(state.alarmStatus == "off") {
    	log.trace "Toggling Alarm - ON - State"
		state.alarmStatus = "on"
        alarmOn()
  	}
	else {
    	log.debug "Toggling Alarm - OFF - State"
		alarmOff()
	}
	
}

def soundAlarm(String uri, volume) {
	log.trace "soundAlarm - Current Value: " + state.alarmStatus
	if(state.alarmStatus == "on") {
    	log.trace "soundAlarm - Playing Track"
